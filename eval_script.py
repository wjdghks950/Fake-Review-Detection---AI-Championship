...
'''
gt = pd.read_csv(sys.argv[1], dtype={"abuse_yn": int, "abuse_class": float}) # 테스트셋정답(sys.argv[1])을 pandas로 불러와 
                               # gt(ground truth)라는 이름으로 할당
pr = pd.read_csv(sys.argv[2], dtype={"abuse_yn": int, "abuse_class": float})  # 채점할결과(sys.argv[2])를 pandas로 불러와 
                               # pr(prediction result) 인자로 할당
print(gt)
print(pr)

##############################################################################
# Preprocess the labels into a single column
# gt = gt.drop(["shop_no", "ord_dt"], axis=1)
gt["abuse_class"] = gt["abuse_class"].fillna(0)
convert_dict = {'abuse_yn': int, 
                'abuse_class': int
               }
gt = gt.astype(convert_dict)
gt.loc[(gt['abuse_yn'] == 1) & (gt['abuse_class'] != 1), 'abuse_yn'] = gt.loc[(gt['abuse_yn'] == 1) & (gt['abuse_class'] != 1), 'abuse_class']
gt = gt.drop("abuse_class", axis=1)
gt = gt.rename(columns={'abuse_yn': 'labels'})
##############################################################################
pr["abuse_class"] = pr["abuse_class"].fillna(0)
convert_dict = {'abuse_yn': int, 
                'abuse_class': int
               }
pr = pr.astype(convert_dict)
pr.loc[(pr['abuse_yn'] == 1) & (pr['abuse_class'] != 1), 'abuse_yn'] = pr.loc[(pr['abuse_yn'] == 1) & (pr['abuse_class'] != 1), 'abuse_class']
pr = pr.drop("abuse_class", axis=1)
pr = pr.rename(columns={'abuse_yn': 'labels'})
...
def eval_metric(y_true, y_pred):
    # Using MCC (Matthew's Correlation Coefficient) - To tackle possible imbalances in the dataset
